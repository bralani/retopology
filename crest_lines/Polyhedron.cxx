...
void Polyhedron::readmesh(int dV, int dF, int maxneighbor, char *filename, char *fileout)
{
  FILE *in = NULL;
  in = fopen(filename, "r");
  ...
  while (myf->setEdgeIDsNext(&dOV1, &dOV2, &edgeFID, &orivID1, &orivID2) == 1)
  {
        ...
        myf->pBox[dOV2]->tconner != 1)
    {
      ...
      if (df2 != -1)
      {
        ...
        {
          if (myf->isDiplicateEdge(edgeFID, df2, ep) == 1)
            checkremove = 1;
        }
      }
      if (df3 != -1)
      {
        if (myf->pointerCheck[df3] != NULL)
        {
          ...
          if (myf->isDiplicateEdge(edgeFID, df3, ep) == 1)
            checkremove = 1;
        }
      }
      if (checkremove == 1)
...
void Polyhedron::MakeNormals(Point3d **dNorma)
{
  ...
  for (i = 0; i < numberV; i++)
  {
    ...
    {
      now = next(now);
      PT->makeVector(bc[0], point[i], point[next(now)->ID]);
      dsize1 = PT->Point3dSize(bc[0]);
      if (dsize1 == 0.0)
        dsize1 = 1.0;
      PT->makeVector(bc[1], point[i], point[now->ID]);
      dsize2 = PT->Point3dSize(bc[1]);
      if (dsize2 == 0.0)
        dsize2 = 1.0;
...
void Polyhedron::SVDFit3Fast()
{
  ...
  for (i = 0; i < numberV; i++)
  {
    ...
    if (i < 1) {
      mysvd->svdcmp(a, 3 * neighborIN[i], (sN - 1), w, v, 1);
    } else {
      mysvd->svdcmp(a, 3 * neighborIN[i], (sN - 1), w, v, 0);
    }
