...
public:
  ...
  int Connect(int p1, int p2, int o1, int o2)
  {
    if (NEhead[p1]->next->ID == p2 ||
        NEhead[p2]->next->ID == p1)
      return -1;
    if (NEhead[p1]->next->ID == NEhead[p2]->next->ID)
      return -1;
  ...
  void makeEdgesN()
  {
    ...
    if (V != 0)
    {
      ...
      while (next(now) != tail)
      {
        ...
        mylist->AppendVF(now->v1, NEtail[now->v2]);
        neighborE[now->v2]++;
        mylist->AppendVF(now->v2, NEtail[now->v1]);
        neighborE[now->v1]++;
      }
    }
  }
  ...
  void PushT(int dv1, int dv2, int dv3, double dx, double dy, double dz, int fID)
  {
    ...
    if (IstEdge(dv1, dv2) == 1)
      removeEdge(dv1, dv2);
    if (IstEdge(dv2, dv3) == 1)
      removeEdge(dv2, dv3);
    if (IstEdge(dv3, dv1) == 1)
      removeEdge(dv3, dv1);
    AppendE(newpid, dv1, fID);
    mylist->AppendVF(newpid, NEtail[dv1]);
    neighborE[dv1]++;
    AppendE(newpid, dv2, fID);
    mylist->AppendVF(newpid, NEtail[dv2]);
    neighborE[dv2]++;
    AppendE(newpid, dv3, fID);
    mylist->AppendVF(newpid, NEtail[dv3]);
    neighborE[dv3]++;
  ...
  void Connect1RingEdge(double Phi, double Theta, int numberV, Point3d **point, Point3d **bc, int *check1v, PointTool *PT, IDList **IHead, IDList **ITail)
  {
    ...
    for (i = 0; i < numberV; i++)
    {
      if (check1v[i] != -1)
      {
        ...
        if (checktj == 1)
        {
          setVec(bc[1], check1v[target]);
          PT->Normalize3D(bc[1]);
          setCandidateVector(bc[2], check1v[i], check1v[target]);
          PT->Normalize3D(bc[2]);
          setCandidateVector(bc[3], check1v[target], check1v[i]);
          PT->Normalize3D(bc[3]);
  ...
  void AppendEadd(int dv1, int dv2)
  {
    ...
    dummy->next = now;
    E++;
    mylist->AppendVF(dv2, NEtail[dv1]);
    neighborE[dv1]++;
    mylist->AppendVF(dv1, NEtail[dv2]);
    neighborE[dv2]++;
  }
